...
//  }
  ...
  ///   - language: язык репозитория
  ///   - order: выбор лучшего совпадения  desc или asc
  private func preparationURL(name: String?, language: String?, order: String) -> URL? {
    var urlComponents = URLComponents()
    urlComponents.scheme = scheme
    urlComponents.host = host
    urlComponents.path = searchRepoPath
  ...
  func getRepositiries(name: String?,
    ...
    guard let url = preparationURL(name: name, language: language, order: order) else { return }
